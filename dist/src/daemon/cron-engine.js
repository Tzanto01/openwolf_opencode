...
export class CronEngine {
    ...
    async runAiTask(params) {
        ...
        const config = readJSON(path.join(this.wolfDir, "config.json"), {});
        const cronConfig = config?.openwolf?.cron ?? {};
        const providers = cronConfig.providers ?? [{ type: "openai_api", priority: 1 }];
    ...
    async callProvider(type, prompt) {
        switch (type) {
            case "openai_api": {
