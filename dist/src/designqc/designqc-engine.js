...
import { findChromePath, captureRouteSectioned, detectRoutes, detectDevServer, startDevServer, } from "./designqc-capture.js";
import { DEFAULT_VIEWPORTS } from "./designqc-types.js";
export class DesignQCEngine {
    wolfDir;
    projectRoot;
    options;
    constructor(wolfDir, projectRoot, options) {
    ...
    async capture() {
        ...
        try {
            ...
            try {
                ...
                for (const route of routes) {
                    for (const viewport of viewports) {
                        if (allScreenshots.length >= this.options.maxScreenshots)
                            break;
                        const url = `${baseUrl}${route}`;
                        console.log(`  Capturing ${route} (${viewport.name}) — full page sections...`);
                        try {
                            const sections = await captureRouteSectioned(page, url, viewport, captureDir, this.options.quality, this.options.maxWidth);
            ...
            for (const s of allScreenshots) {
                try {
                    totalSizeBytes += fs.statSync(s.path).size;
                }
                catch { }
            }
            ...
            appendText(path.join(this.wolfDir, "memory.md"), `| ${timeStr} | designqc: captured ${allScreenshots.length} screenshots (${totalSizeKB}KB, ~${estimatedTokens} tok) | ${routes.join(", ")} | ready for eval | ~0 |\n`);
...
//# sourceMappingURL=designqc-engine.js.map